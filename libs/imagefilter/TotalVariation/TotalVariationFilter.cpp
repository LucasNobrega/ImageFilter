...
TotalVariationFilter& TotalVariationFilter::operator=(const TotalVariationFilter& other) {
    if(this != &other){
...
bool TotalVariationFilter::apply()
{
    ...
    kernel[0] = (cv::Mat_<double>(kernel_size,kernel_size) << -1, 0, 0, 0, 1, 0, 0, 0, 0);
    kernel[1] = (cv::Mat_<double>(kernel_size,kernel_size) << 0, -1, 0, 0, 1, 0, 0, 0, 0);
    kernel[2] = (cv::Mat_<double>(kernel_size,kernel_size) << 0, 0, -1, 0, 1, 0, 0, 0, 0);
    kernel[3] = (cv::Mat_<double>(kernel_size,kernel_size) << 0, 0, 0, -1, 1, 0, 0, 0, 0);
    kernel[4] = (cv::Mat_<double>(kernel_size,kernel_size) << 0, 0, 0, 0, 1, -1, 0, 0, 0);
    kernel[5] = (cv::Mat_<double>(kernel_size,kernel_size) << 0, 0, 0, 0, 1, 0, -1, 0, 0);
    kernel[6] = (cv::Mat_<double>(kernel_size,kernel_size) << 0, 0, 0, 0, 1, 0, 0, -1, 0);
    kernel[7] = (cv::Mat_<double>(kernel_size,kernel_size) << 0, 0, 0, 0, 1, 0, 0, 0, -1);

    for(int i=0 ; i<8 ; i++){
        // Apply 2D filter
        cv::filter2D(*raw_image, outputi, ddepth, kernel[i], anchor, delta);

        std::cout << "filtere_image: " << filtered_image->rows << ", " << filtered_image->cols << std::endl;
        std::cout << "output_i: " << outputi.rows << ", " << outputi.cols << std::endl;
