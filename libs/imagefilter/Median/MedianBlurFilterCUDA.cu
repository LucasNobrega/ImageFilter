...
__device__ void sortCUDA(int filter_width,
                         int filter_height,
                                       ...
                                       int channel)
{
        ...
        (y >= filter_height / 2) && (y < (height - filter_height / 2)))
    {
        for (int c = 0; c < channel; c++)
        {
            ...
            unsigned char *filterVector = static_cast<unsigned char *>(
            ...
            for (int ky = -filter_height / 2; ky <= filter_height / 2; ky++)
            {
                for (int kx = -filter_width / 2; kx <= filter_width / 2; kx++)
                {
                    ...
                    filterVector[((ky * filter_width + kx) + size) % size] =
                        srcImage[((y + ky) * width + (x + kx)) * channel + c];
                }
            }
            // Sorting values of filter
            sortCUDA(filter_width, filter_height, filterVector);
            dstImage[(y * width + x) * channel + c] =
                filterVector[(filter_width * filter_height) / 2];
            free(filterVector);
        }
    }
}
...
MedianBlurFilterCUDA &MedianBlurFilterCUDA::operator=(
    const MedianBlurFilterCUDA &other)
{
    if (this != &other)
    {
