            ...
            for(int ky = -filter_height/2; ky <= filter_height/2; ky++) {
                for(int kx = -filter_width/2; kx <= filter_width/2; kx++) {
                    ...
                    if ((x + kx) >= 0 && (x + kx) < width && (y + ky) >= 0 && (y + ky) < height) {
                        ...
                        sum += fl * kernel[((ky + filter_height/2) + filter_height) % filter_height]
                                          [((kx + filter_width/2) + filter_width) % filter_width];
                    }
                }
            }
...
SharpeningFilterCUDA::SharpeningFilterCUDA( unsigned int filter_kernel_width,
...
SharpeningFilterCUDA& SharpeningFilterCUDA::operator=(const SharpeningFilterCUDA& other) {
    if(this != &other){
